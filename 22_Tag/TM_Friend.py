...
def find_tag_idx(sent):
    ...
    return tag_found, tag_found_idx
 
def test_excel(which_list):
    timestamp = datetime.now().strftime('%m%d%H%M')
    workbook = xlsxwriter.Workbook('./results/simple/simple_regex_test_' + timestamp + '_.xlsx')
    worksheet = workbook.add_worksheet()
    worksheet.write('A1', 'Raw_TM')
    worksheet.write('B1', 'Tag')
    worksheet.write('C1', 'Tag_idx')
    row_idx = 2 

    for sent in which_list:
        ...
        tag_found, tag_found_idx = find_tag_idx(sent)
        for idx in range(len(tag_found)):
            tag = tag_found[idx]
            tag_idx = tag_found_idx[idx]
            worksheet.write(b_idx, tag)
            worksheet.write(c_idx, tag_idx)
            row_idx += 1
