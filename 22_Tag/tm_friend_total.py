...
# tm 불러오기
from tm_to_list import call_tagMT_ko_total_lan
...
def tm_total_extract_excel(which_list):
    start = timeit.default_timer()
    timestamp = datetime.now().strftime('%m%d%H%M')

    ########### error ###########
    workbook_error = xlsxwriter.Workbook('./results/total/error_total_'+ lan + '_' + timestamp + '_.xlsx')
    ...
    ########### regular ###########
    workbook_normal = xlsxwriter.Workbook('./results/total/normal_total_' + lan + '_' + timestamp + '_.xlsx')
    ...
    for idx, sent in enumerate(which_list):
        ...
        # 길이가 맞을 때
            ...
            worksheet_normal.write(a_idx, path_total_list[idx])
...
tm_total_extract_excel(lan_list)
