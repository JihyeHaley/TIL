...
def docx_to_raw_text(file_path, stop_words):
    ...
    for line in contents:
        ...
        if each_line and not is_meaningful(each_line) and \
                each_line not in stop_words and not is_page_num(each_line) and not neitherKoNorEn(each_line):
...
def get_filename_list(root, child, ext):
    ...
    for sub in sub_list:
        ...
        if not sub_dir.strip():
            ...
            Path('./results/' + child + '/' + sub_dir + "_output").mkdir(parents=True, exist_ok=True)
