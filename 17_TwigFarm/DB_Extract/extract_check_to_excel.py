...
def word_extract_to_word(pdf_filtered_dict, pdf_failed_dict, sub_path):
    ...
    # analyze and write
    # dict안에 있는 개수 별로
    if len(pdf_filtered_dict) == len(pdf_failed_dict):
        for key, value in pdf_filtered_dict.items():
            start = timeit.default_timer() # 작업 시작 시점

            #########################################################
            # db 처리 엑셀 파일
            db_workbook = xlsxwriter.Workbook('./results/' + sub_path + '/' + key + '_pdf_추출문장_'  + timestamp +'.xlsx') 
            ...
            #########################################################
            # db 비처리 엑셀 파일
            failed_workbook = xlsxwriter.Workbook('./results/' + sub_path + '/' + key + '_pdf_비추출문장_'  + timestamp +'.xlsx') 
            ...
            #########################################################
            # Failed_list, analyze and write
            pdf_failed_list = pdf_failed_dict[key]
            # 추출하지 않은 모든 파일 다시 적기
