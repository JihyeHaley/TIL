def file_to_excel():
    ...
    for pptx_name_list in pptx_name_lists:
        ...
        pptx_results_pre = reg_raw_text(pptx_results_pre)
        
        pptx_results = list(filter(None, pptx_results_pre))

        if i == 1:
            pptx_test_print_1 = pptx_name_list[40:-5]
        else:
            print(i, end=" ")
    # ------------------------------------------------------------------------------------
    # docx dict 만들기
    # 잠시보류
        file_lists_pptx[pptx_name_list[40:-5]] = pptx_results
    ...
    # ------------------------------------------------------------------------------------
    # log 출력 (파일명 확인하기 위해서)
    completed_log = open(f'/Users/jihyeoh/Desktop/Farm/4_2019한국표준협회/log_stabdard_33' + '.txt', "w+")
    ...
    # ------------------------------------------------------------------------------------
    # excel 밖으로 빼내기
    workbook = xlsxwriter.Workbook('/Users/jihyeoh/Desktop/Farm/4_2019한국표준협회/4_xlsx/standard33_.xlsx')
