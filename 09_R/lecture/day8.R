...
apply(df, 1, sum, na.rm=TRUE) #na.rm 은 sum한테 전달이 된다.
apply(df, 2, sum, na.rm=TRUE) #? 1,2??
lapply(df, sum, na.rm=TRUE) #list로 리턴 열단위로
sapply(df, sum, na.rm=TRUE) #vector로 리턴 
tapply(1:6, gender, sum, na.rm=TRUE)
tapply(df$w, gender, mean, na.rm=TRUE)
mapply(paste, 1:5, LETTERS[1:5], month.abb[1:5])

v<-c("abc", "DEF", "TwT")
sapply(v, function(d) paste("-",d,"-", sep="")) #named vector

l<-list("abc", "DEF", "TwT")
sapply(l, function(d) paste("-",d,"-", sep="")) #vector로 리턴
lapply(l, function(d) paste("-",d,"-", sep="")) #그때 그때 다른 return

flower <- c("rose", "iris", "sunflower", "anemone", "tulip")
length(flower)
nchar(flower) #문자길이
sapply(flower, function(d) if(nchar(d) > 5) return(d)) #마지막으로 수행한 값이 결과로 리턴
...
sapply(flower, function(d, n) if(nchar(d) > n) return(d), 2)
