...
print(ft(100))#a만 틀림! function(a)이고 a가 100이라서 100 출력력
print(a);print(b);print(c);print(d) 

#invisible()함수 #just look like nothing showed up?

ft.1 <- function(x) return()
ft.2 <- function(x) return(x+10)
ft.3 <- function(x) invisible(x+10) 
#literally does not show the return result
...
testParamType(data.frame()) # is.list도 참이되네
testParamType(matrix())     #is.array도 참이되네
                            #matrix는 2차원 array..
testParamType(list())       #is.vector도 참이되네
                            #list는 벡터의 확장 
testParamType(array())
testParamType(mean)
...
test1 <-function(p){
  cat("난 수행함\n")
  testError(-1) #위에꺼는 testError1
  cat("나 수행할 까요? \n")
}
test1()

# try() 건너뛰기 
# - 시행중에 에러가 나더라도 다음으로 넘어가겠어
test2 <- function(p){
  cat("난 수행함\n")
  try(testError(-1))
  cat("나 수행할 까요? \n")
